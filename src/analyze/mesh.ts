...
export function analyzeMeshes(spineInstance: Spine) {
    ...
    skeleton.slots.forEach((slot) => {
        if (slot.getAttachment() && slot.getAttachment().type === AttachmentType.Mesh) {
...
function appendMeshMisuseInfo(slotName: string, isUsedInMeshSequence: boolean): void {
