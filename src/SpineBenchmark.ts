...
export class SpineBenchmark {
    ...
    public loadSpineFiles(files: FileList) {
        ...
        const getFilename = (str: string) => str.substring(str.lastIndexOf('/') + 1);
    ...
    private createSpineAsset(data: any, atlasText: string): void {
        ...
        setTimeout(() => {
            ...
            const camera: any = this.app.stage.children[0];

            // Remove previous Spine instance if exists
            if (this.spineInstance) {
                camera.removeChild(this.spineInstance as any);
            }

            camera.addChild(skeleton as any);
            camera.lookAtChild(skeleton as any);
