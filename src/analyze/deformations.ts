...
export function analyzeDeformations(spineInstance: Spine, threshold = 0.1) {
    ...
    animations.forEach((animation) => {
        ...
        const meshLoad = Math.floor(
            meshTransformations
                .map((a) => a.changes.reduce((partialSum, a) => partialSum + a.difference, 0))
    ...
    const mergedMap = mergeMaps(['load1', 'duration'], animationMeshLoads, animationDurations);
    const table = createTable(mergedMap, ['Animation', 'Changes', 'Duration']);
...
function createTable(mergedMap: Map<string, Record<string, any>>, columns: string[]) {
